...
AppMobi.toolkit.ui is a UI library for building mobile apps
...
== How to use AppMobi.toolkit.ui

To use AppMobi.toolkit.ui, you need to include the javascript files.  We suggest using appMobiUI.min.js which has all the necesary files, otherwise include the source for all of them, 
along with the CSS.
...
* content - this is the content area.  You do not need to add anything here.  All page divs will be added here.  If you put a div in there, it will be shown first.
    ...
    Each div inside will need the class "panel".  AppMobi.toolkit.ui will search for all elements with that classname and add them automatically.
...
* AppMobi.toolkit.scroller is added automatically, along with AppMobi.toolkit.appMobiSelect, ApppMobi.toolkit.appMobiPassword, and AppMobi.toolkit.css3animate.
...
* To update content, you must call the function updateContentDiv(_id_,_content_);
  ...
  where AMUi is a new AppMobi toolkit ui 
    var AMUi = new AppMobi.toolkit.ui();
...
* Please see appMobiUI.css for additional button colors and ways to change the theme				