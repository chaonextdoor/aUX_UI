...
prevTime:0,finishScrollingObject:null,container:null,maxLeft:0,startLeft:0,rightMargin:0,divWidth:0,touchMove:function(c){try{if(d||(d=!0,this.touchStart(c)),this.currentScrollingObject!=null){c.preventDefault();var b={x:0,y:0},a=0,e=0,f=0,g=0;if(this.verticalScroll){var p=this.lockY-c.touches[0].pageY,p=-p,a=this.startTop+p,e=numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).f);b.y=a}if(this.horizontalScroll){var m=this.lockX-c.touches[0].pageX,m=-m,p=f=this.startLeft+
m,g=-numOnly((new WebKitCSSMatrix(window.getComputedStyle(this.el).webkitTransform)).e);b.x=p}this.scrollerMoveCSS(this.currentScrollingObject,b,0);if(this.vscrollBar){var q=this.bottomMargin-numOnly(this.vscrollBar.style.height)-(this.maxTop+a)/this.maxTop*(this.bottomMargin-numOnly(this.vscrollBar.style.height));this.scrollerMoveCSS(this.vscrollBar,{x:0,y:q},0)}this.hscrollBar&&(q=this.rightMargin-numOnly(this.hscrollBar.style.width)-(this.maxLeft+f)/this.maxLeft*(this.rightMargin-numOnly(this.hscrollBar.style.width)),
this.scrollerMoveCSS(this.hscrollBar,{x:q,y:0},0));if(this.prevTime){var t=Math.abs(e)-Math.abs(a),s=Math.abs(g)-Math.abs(f),o=c.timeStamp-this.prevTime;o<1E3?(this.timeMoved+=o,this.vdistanceMoved+=t,this.hdistanceMoved+=s):this.hdistanceMoved=this.vdistanceMoved=this.timeMoved=0}this.prevTime=c.timeStamp}}catch(n){alert("error in scrollMove: "+n)}},touchStart:function(c){var b=this.container,a=this.el;if(b)try{if(c.touches[0].target&&c.touches[0].target.type!=void 0){var e=c.touches[0].target.tagName.toLowerCase();
if(e=="select"||e=="input"||e=="button")return}this.hdistanceMoved=this.vdistanceMoved=this.timeMoved=0;this.finishScrollingObject=this.prevTime=null;this.bottomMargin=b.clientHeight>window.innerHeight?window.innerHeight:b.clientHeight;this.maxTop=a.clientHeight-this.bottomMargin;this.divHeight=a.clientHeight;this.rightMargin=b.clientWidth>window.innerWidth?window.innerWidth:b.clientWidth;this.maxLeft=a.clientWidth-this.rightMargin;this.divWidth=a.clientWidth;if(!(this.maxTop<0)&&c.touches.length==
1&&this.boolScrollLock==!1){try{this.startTop=numOnly((new WebKitCSSMatrix(window.getComputedStyle(a).webkitTransform)).f),this.startLeft=numOnly((new WebKitCSSMatrix(window.getComputedStyle(a).webkitTransform)).e)}catch(d){this.startLeft=this.startTop=0,console.log("error scroller touchstart "+d)}this.lockX=c.touches[0].pageX;this.lockY=c.touches[0].pageY;this.currentScrollingObject=a;if(this.vscrollBar){this.vscrollBar.style.height=parseFloat(this.bottomMargin/this.divHeight)*this.bottomMargin+
"px";var f=this.bottomMargin-numOnly(this.vscrollBar.style.height)-(this.maxTop+this.startTop)/this.maxTop*(this.bottomMargin-numOnly(this.vscrollBar.style.height));this.scrollerMoveCSS(this.vscrollBar,{x:0,y:f},0);this.container.clientWidth>window.innerWidth?this.vscrollBar.style.left=window.innerWidth-numOnly(this.vscrollBar.style.width)+"px":this.vscrollBar.style.right="0px";this.vscrollBar.webkitTransition="opacity";this.vscrollBar.style.opacity=1}if(this.hscrollBar)this.hscrollBar.style.width=
